...
if(isset($_GET['id']) && $_GET['id']!=''){
	...
	$res=mysqli_query($con,"select * from tuitor_admin where id='$id'");
...
if(isset($_POST['submit'])){
	...
	$res=mysqli_query($con,"select * from tuitor_admin where email='$email'");
	$check=mysqli_num_rows($res);
	if($check>0){
		if(isset($_GET['id']) && $_GET['id']!=''){
	...
	if($msg==''){
		if(isset($_GET['id']) && $_GET['id']!=''){
			mysqli_query($con,"update tuitor_admin set email='$email', password='$password',lname='$lname',fname='$fname',mobile='$mobile', where id='$id'");
		}else{
			$added_on=date('Y-m-d h:i:s');
			mysqli_query($con,"insert into tuitor_admin(email, password, fname, lname, mobile,added_on) 
